...
public class AHotkeysListener extends MouseAdapter implements KeyListener
{
	...
	public void keyPressed(KeyEvent e)
	{
		...
		try
		{
				...
				if (config.type1() == type1.OBJECT) {
					...
					clientThread.invoke(() ->
							client.invokeMenuAction("", "", obj.getId(),
									config.obj1().action.getId(), obj.getSceneMinLocation().getX(), obj.getSceneMinLocation().getY()));
				}
			...
			if (key_code == config.key6().getKeyCode())
			{
			...
			if (key_code == config.key7().getKeyCode())
			{
			...
			if (key_code == config.key8().getKeyCode())
			{
			...
			if (key_code == config.key9().getKeyCode())
			{
			...
			if (key_code == config.key10().getKeyCode())
			{

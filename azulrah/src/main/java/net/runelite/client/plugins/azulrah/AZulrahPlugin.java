...
public class AZulrahPlugin extends Plugin
{
			...
			switch (state) {
				...
				case FAIRY_RING:
					if (!utils.isItemEquipped(Collections.singleton(772))){
						clientThread.invoke(() -> client.invokeMenuAction("Wield", "<col=ff9040>" + 772, 772, MenuAction.ITEM_SECOND_OPTION.getId(), utils.getInventoryWidgetItem(772).getIndex(), WidgetInfo.INVENTORY.getId()));
					}
	...
	private void onNpcChanged(NpcChanged event) {
		...
		if (npcId == 2042 && !configph.RangedOnly()) {
			Widget inventory = client.getWidget(WidgetInfo.INVENTORY);
			if (inventory == null) {
				return;
			}
		...
		} else if (npcId == 2043 && !configph.RangedOnly()) {
			Widget inventory = client.getWidget(WidgetInfo.INVENTORY);
			if (inventory == null) {
				return;
			}
		...
		} else if (npcId == 2044 && !configph.MageOnly()) {
			Widget inventory = client.getWidget(WidgetInfo.INVENTORY);
			if (inventory == null) {
				return;
			}
